...
class ProductStoreRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $baseRules = [
            ...
            'product_type' => ['required', 'string', 'in:deposit,loan,wallet,escrow'],
    ...
    protected function getProductTypeSpecificRules(): array
    {
        ...
        switch ($productType) {
            case 'deposit':
                return [
                    'minimum_amount' => ['required', 'numeric', 'min:0', 'lt:maximum_amount'],
                    'maximum_amount' => ['required', 'numeric', 'min:0'],
                    'interest_rate' => ['required', 'numeric', 'min:0', 'max:100'],
                    'interest_rate_type' => ['required', 'string', 'in:fixed,variable'],
                    'interest_calculation_frequency' => ['required', 'string', 'in:daily,monthly,annually'],
                    'interest_posting_frequency' => ['required', 'string', 'in:monthly,quarterly,annually'],
            ...
            case 'loan':
                return [
                    ...
                    'interest_rate' => ['required', 'numeric', 'min:0', 'max:100'],
                    'interest_rate_type' => ['required', 'string', 'in:fixed,variable'],
                    'repayment_frequency' => ['required', 'string', 'in:daily,weekly,monthly,annually'],
                    'amortization_type' => ['required', 'string', 'in:flat,declining_balance'],
                    'grace_period_days' => ['required', 'integer', 'min:0'],
                    'late_payment_penalty_rate' => ['required', 'numeric', 'min:0', 'max:100'],
            ...
            case 'escrow':
                return [
                    'minimum_amount' => ['nullable', 'numeric', 'min:0', 'lt:maximum_amount'],
